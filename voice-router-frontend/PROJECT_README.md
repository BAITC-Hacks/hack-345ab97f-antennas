...
> Этот документ описывает целевую архитектуру команды, не готовность текущей ветки. Docker, make eval, STT/TTS, audio playback, очередь операторов и серверный каталог здесь не реализованы. Рабочий запуск frontend: `cd voice-router-frontend`, затем `node server.mjs`; точный статус — в [README.md](./README.md) и [CONFORMANCE.md](./CONFORMANCE.md).
