...
import { recordedPcm16 } from './audio'
...
const sampleRu = isDemo
  ? 'Я вчера оплатил, деньги списались, но заказ не подтвердился. И ещё адрес доставки поменять.'
  : 'Я оплатил страховку, деньги списались, но полис не пришёл. И ещё хочу поменять адрес доставки полиса.'
const sampleKk = isDemo ? 'Төлем жасадым, бірақ тапсырыс расталмады.' : 'Полисімнің мерзімі бітіп жатыр, ұзартқым келеді.'
const sampleMixed = isDemo ? 'Заказды оплатил, бірақ адрес доставки өзгерткім келеді.' : 'Страховканы оплатил, бірақ полис әлі жоқ.'
...
function CallPage({ scenarios, dialogs, onDialog, onTrace }: {
  ...
  useEffect(() => () => {
    ...
    if (callIdRef.current) void api.endCall(callIdRef.current).catch(() => undefined)
  }, [])
  ...
  async function startRecording() {
    if (isDemo || workingRef.current) return
    ...
    try {
      ...
      setMicActive(true)
      setMicMessage('Идёт запись · до 30 секунд')
      setStatus('listening')
    } catch (cause) {
      stream?.getTracks().forEach((track) => track.stop())
      setMicMessage(cause instanceof Error ? cause.message : 'Микрофон недоступен · используйте текст')
    }
  }

  async function stopRecording(): Promise<Blob> {
    const recorder = recorderRef.current
    if (!recorder || recorder.state !== 'recording') throw new Error('Нет активной записи.')
    const blob = await new Promise<Blob>((resolve, reject) => {
      recorder.addEventListener('stop', () => resolve(new Blob(chunksRef.current, { type: recorder.mimeType })), { once: true })
      recorder.addEventListener('error', () => reject(new Error('Ошибка записи с микрофона.')), { once: true })
      recorder.stop()
    })
    releaseMicrophone()
    return blob
  ...
  async function connect(withMicrophone: boolean): Promise<string | null> {
    ...
    try {
      ...
      setCallId(id)
      setStatus('listening')
      if (withMicrophone) await startRecording()
      return id
    } catch (cause) {
      releaseMicrophone()
      setStatus('error')
  ...
  function handleEvent(event: CallEvent) {
    ...
    if (event.type === 'reply') {
      setReply(event.text)
      if (event.ttsUrl) {
        audioRef.current = new Audio(event.ttsUrl)
        void audioRef.current.play().catch(() => setTtsMessage('Аудио недоступно, текст ответа сохранён.'))
      }
    }
    if (event.type === 'tts_audio') {
      ttsReceivedRef.current = true
      audioRef.current?.pause()
      audioRef.current = new Audio(`data:audio/wav;base64,${event.chunk}`)
      void audioRef.current.play()
        .then(() => setTtsMessage('Голосовой ответ воспроизводится'))
        .catch(() => setTtsMessage('Браузер заблокировал воспроизведение · текст доступен'))
    }
    if (event.type === 'error') {
      if (event.message.startsWith('TTS:')) {
        ttsErrorRef.current = true
        setTtsMessage('Аудио недоступно · текст ответа сохранён')
      }
  ...
  function resetTurn() {
    ...
    ttsErrorRef.current = false
    audioRef.current?.pause()
  }

  function showDialog(result: Dialog) {
    setTranscript(result.transcript)
    setRoute(result.route)
    setReply(result.reply)
    setDialog(result)
    setStatus('completed')
    if (!ttsReceivedRef.current && !ttsErrorRef.current) setTtsMessage('Текстовый ответ · TTS выключен')
    onDialog(result)
  }
  ...
  async function sendText(value = input) {
    ...
    if (micActive) releaseMicrophone()
    setInput('')
    resetTurn()
  ...
  async function sendRecording() {
    if (!callId || !api.submitAudio || workingRef.current) return
    workingRef.current = true
    resetTurn()
    ...
    try {
      ...
      const pcm16 = await recordedPcm16(blob)
      setMicMessage('Запись отправлена на распознавание')
      const result = await api.submitAudio(callId, pcm16, handleEvent, controller.signal)
      showDialog(result)
    } catch (cause) {
      if (!controller.signal.aborted) {
        setStatus('error')
  ...
  return <div className="page">
      ...
      <section className="panel call-panel">
        ...
        <div className="call-center">
          ...
          <div className="call-actions">
            {!callId ? <button className="primary-button large" onClick={() => void connect(!isDemo)}>◉ &nbsp; Начать звонок</button>
              : <button className="danger-button large" onClick={() => void endCall()}>■ &nbsp; Завершить звонок</button>}
            {callId && !isDemo && !micActive && <button className="outline-button" disabled={busy} onClick={() => void startRecording()}>◉ &nbsp; Записать реплику</button>}
          ...
          {micActive && <div className="record-actions"><button className="primary-button" disabled={busy} onClick={() => void sendRecording()}>Отправить запись ↗</button><button className="text-button" onClick={() => { releaseMicrophone(); setMicMessage('Запись отменена') }}>Отменить</button></div>}
        </div>
        ...
        <div className="input-area">
          ...
          <form onSubmit={(event) => { event.preventDefault(); void sendText() }} className="message-form">
            <textarea aria-label="Реплика клиента" placeholder={isDemo ? 'Например: деньги списались, но заказ не подтвердился…' : 'Например: оплатил страховку, но полис не пришёл…'} value={input} onChange={(event) => setInput(event.target.value)} rows={3} />
    ...
    {error && <div role="alert" className="alert error-alert">{error} {status === 'disconnected' && <button onClick={() => { void endCall().then(() => connect(false)) }}>Подключиться снова</button>}</div>}
...
function CatalogPage({ scenarios, onSaved }: { scenarios: Scenario[]; onSaved: (scenario: Scenario) => void }) {
  ...
  async function save(event: FormEvent) {
    ...
    if (!/^[a-z][a-z0-9_]{1,63}$/.test(next.id)) { setError('ID: 2–64 символа, первая — латинская буква; далее буквы, цифры и _.'); return }
