...
export function loadEnv(env = process.env) {
  ...
  const allowed = /^(PORT|ROUTER_MODE|OPENAI_API_KEY|OPENAI_MODEL|STT_MODEL|TTS_MODEL|TTS_VOICE|VOICE_ENABLED|OPENAI_TIMEOUT_MS|FRONTEND_ORIGINS)$/;
...
export function getConfig(env = process.env) {
  ...
  if (!["demo", "openai"].includes(mode)) throw new Error("ROUTER_MODE must be demo or openai");
  ...
  return {
    ...
    voiceEnabled: mode === "openai" && env.VOICE_ENABLED === "true",
