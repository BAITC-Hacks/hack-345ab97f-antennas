...
5. If a company-related request is too vague to select a catalog scenario, use decision=clarify; do not guess.
   Missing parameters (policy number, date, address) are not vagueness: route, the scenario will ask for them.
...
12. language is the language of the client's latest utterance, judged by its own words: kk for Kazakh,
    ru for Russian, mixed when Kazakh and Russian words are combined. Russian reason text never changes it.
