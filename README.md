...
> **Статус на 23 сентября 2026 года:** в `main` объединены локальный Node.js Gateway (`voice-router-backend/`) и совместимый интерфейс (`voice-router-frontend/`); сохранены React/Vite-интерфейс (`frontend/`), Python LLM-роутер (`backend/app/router/`), инструменты оценки (`eval/`) и учебный прототип (`router_prototype/`) команды. Сквозное локальное демо Node.js, каталог, история и очередь карточек проверены. Реальные OpenAI/STT/TTS-вызовы новым ключом, качество речи и финальный eval ещё требуют проверки. Архитектурный план ниже не означает готовность production.

Репозиторий команды: [BAITC-Hacks/hack-345ab97f-antennas](https://github.com/BAITC-Hacks/hack-345ab97f-antennas). Docker Compose пока не добавлен; каталог и тестовые JSON-наборы находятся в `data/` и `eval/`. Два интерфейса сохранены отдельно: новый Gateway совместим с `voice-router-frontend/`; live-контракт React-приложения `frontend/` ещё нужно согласовать, он не подключается автоматически. Python-роутер также пока не подключён к Node.js Gateway.
...
Локальный каталог и последние трассы сохраняются в `voice-router-backend/data/state.json`. Файлы `.env`, содержимое `voice-router-backend/data/` (включая `.lock`) и `node_modules/` не публикуются в Git. Корневая папка `data/` содержит отдельный версионируемый каталог и наборы команды, а не runtime-данные Gateway. Используйте синтетические данные.

### Python LLM-роутер команды

В `backend/app/router/` находится отдельная реализация роутера; её [контракт](docs/contracts/router.md) и [методика оценки](eval/README.md) сохранены без изменения. Учебный [прототип](router_prototype/README.md) также включён в `main`. Эти компоненты не заменяют автоматически роутер Node.js Gateway.

Автономные Python-тесты из корня репозитория (Python 3.11+, PowerShell):
...
Это эквивалент `make test`, без обращений к модели. Реальный eval запускается отдельно по инструкции `eval/README.md`, требует ключа и может расходовать средства. При этом слиянии платный eval не запускался, holdout не использовался для оценки или настройки.
...
| --- | --- | --- |
| Top-1 accuracy, размер набора | Прототип: 24/24 на небольшом тестовом наборе; финальный eval ожидается | `router_prototype/reports/2026-09-23_1403_gpt-4.1-mini.json` |
